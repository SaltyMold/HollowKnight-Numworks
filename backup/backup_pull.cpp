...
int main(void) {
	...
	for (int i = 0; i < 4; i++) {
		DRAW_IMAGE_TRANSPARENT(Point(i * 80, 240 - 50), Image::FloorStone80x10);
	}
	...
	while (1) {
		...
		if (state.keyDown(Keyboard::Key::Left)) {
			...
			xKnight--;
			xCamera--;
			knightRect = Rect(xKnight, yKnight, knightW, knightH);
			eadk_display_pull_rect(knightRect, bgBuffer);
			DRAW_IMAGE_TRANSPARENT(Point(xKnight, yKnight), Image::KnightAFK14x30);
		}
